...
name_file = open("logged.txt", "r")
name_logged = name_file.readline()

path = ".\\Dependencies\\User Data\\"
list_files = os.listdir(path)

workbook_location = f"{path}{name_logged}.xlsx"
if (f"{name_logged}.xlsx") in list_files:
    pass
else:
    wb = openpyxl.Workbook()
    wb.save(workbook_location)

folder_location_main = ".\\Dependencies\\images\\ContactMainScreen\\"


def destroy(): window.destroy()


def add_contact(name="*Full Name", contact="*Contact", category="Work/Family/Friend/Other", email="E-mail", dob="DOB:DD-MM-YYYY", profile_path=""):
    ...
    def submit(*args):
        ...
        try:
            ...
            workbook1 = openpyxl.load_workbook(workbook_location)
            worksheet1 = workbook1.active         

            path = ".\\Dependencies\\User Data\\" + name_logged + "\\"
            ...
            if name1 + str("_see.png") in list_of_files:
                PROFILE_LOCATION = ".\\Dependencies\\User Data\\" + name_logged + "\\" + str(
                    name1) + "_see.png"
                

            else:
                PROFILE_LOCATION = ".\\Dependencies\\User Data\\Default.png"

            if name1 == "" and contact1 == "" or contact1 == "" or name1 == "":
                messagebox.showerror("IMP", "Required field are missing")
            

            if dob1 == 'DOB:DD-MM-YYYY' or dob1 == "":
                dob1 = "N/A"
            ...
            temp_list.append(contact1)
            temp_list.append(email1)
            temp_list.append(dob1)
            temp_list.append(Category1)
            temp_list.append(PROFILE_LOCATION)

            worksheet1.append(temp_list)
            workbook1.save(workbook_location)
    ...
    def edit_submit():
        ...
        path = f".\\Dependencies\\User Data\\{name_logged}\\"
        list_of_files = os.listdir(path)
        ...
        if name + str("_see.png") in list_of_files:
            PROFILE_LOCATION1 = ".\\\\Dependencies\\\\User Data\\\\" + name_logged + "\\\\" + str(
                name1) + "_see.png"
        else:
            PROFILE_LOCATION1 = ".\\\\Dependencies\\\\User Data\\\\Default.png"
        ...
        worksheet1[f'A{row}'] = name1
        worksheet1[f'B{row}'] = contact1
        worksheet1[f'C{row}'] = email1
        worksheet1[f'D{row}'] = dob1
        worksheet1[f'E{row}'] = Category1
        worksheet1[f'F{row}'] = PROFILE_LOCATION1
    ...
    def change(e):
        ...
        change_path =  ".\\Dependencies\\images\\Add_Contact\\Profile_Change.png"
    ...
    def undo_change(e):
        ...
        if name != "*Full Name":
            ...
            if namecont + str(".png") in list_of_files:
                cpath =  ".\\Dependencies\\User Data\\" + name_logged + "\\" + str(namecont) + str(".png")
                default_pic1 = PhotoImage(file=cpath)
                default_pic.config(image=default_pic1)

            else:
                defprofile_path = ".\\Dependencies\\images\\Add_Contact\\Contact image.png"
    ...
    def pic_change():
        ...
        try:
            x = str(".\\Dependencies\\User Data\\") + str(name_logged) + str("\\") + str(
                namecont) + ".png"
            y = str(".\\Dependencies\\User Data\\") + str(name_logged) + str("\\") + str(
                namecont) + "_see.png"
            os.remove(x)
            os.remove(y)
        except:
        ...
        try:
            ...
            resize_path = str(".\\Dependencies\\User Data\\") + str(name_logged) + str("\\") + str(
                namecont) + "_resize.png"
            paste_path = str(".\\Dependencies\\User Data\\") + str(name_logged) + str("\\") + str(
                namecont) + ".png"
            see_save_path = str(".\\Dependencies\\User Data\\") + str(name_logged) + str("\\") + str(
                namecont) + "_see.png"
    ...
    # Background Pic
    logo = PhotoImage(file = ".\\Dependencies\\images\\Add_Contact\\new_BG.png")
    label2 = Label(addwindow, image=logo, border=0)
    label2.place(x=0, y=0)

    # ProfileButton
    if name != "*Full Name" and profile_path != ".\\Dependencies\\User Data\\Default.png":
        defprofile_path = ".\\Dependencies\\User Data\\" + str(name_logged) + "\\" + name + ".png"
    ...
    # Submit Button
    submit_path = ".\\Dependencies\\images\\Add_Contact\\save_1.png"
    ...
    # Cancel Button
    cancel_path = ".\\Dependencies\\images\\Add_Contact\\cancel_1.png"
...
def see_contact():
    ...
    # ImagePaths
    folder_location = ".\\Dependencies\\images\\SeeContact\\"
    ...
    # Folder Creation
    file_path1 = ".\\Dependencies\\User Data\\"
    try:
    ...
    def delete(name):
        ...
        for i in range(1, worksheet1.max_row+1):
            row = [cell.value for cell in worksheet1[i]]
            if row[0] != name:     
                temp_list.append(row)
        
        tempNewWB = openpyxl.Workbook()
        tempWS = tempNewWB.active

        for rows in temp_list:
            tempWS.append(rows)

        tempNewWB.save(workbook_location)
    ...
    def mainscreen(cat="None"):
        ...
        name_list, contact_list, email_list, dob_list, category_list, profile_location, show_result = [], [], [], [], [], [], []
        ...
        show_result.sort(key=lambda  x: x[0] )
        ...
        LBG_path = ".\\Dependencies\\images\\SeeContact\\LabelBG.png"
...
def search_():
    ...
    def search_command(*args):
        ...
        for i in range(1, worksheet1.max_row+1):
            ...
            if search == "":
                break
            elif search.lower() in row[0] or search.upper() in row[0] or search in row[0]:
                show_result.append(row)
